...
layer parse_deconvolutional(list *options, size_params params, int count)
{
    int n = option_find_int(options, "filters",1);
    int size = option_find_int(options, "size",1);
    int stride = option_find_int(options, "stride",1);
    ...
    layer l = make_deconvolutional_layer(batch,h,w,c,n,size,stride,padding, activation, batch_normalize, params.net->adam, count);
...
layer parse_yolo(list *options, size_params params, int count)
{
...
maxpool_layer parse_maxpool(list *options, size_params params, int count)
{
    int stride = option_find_int(options, "stride",1);
...
layer parse_shortcut(list *options, size_params params, network *net, int count )
{
...
layer parse_upsample(list *options, size_params params, network *net, int count)
{
...
route_layer parse_route(list *options, size_params params, network *net, int count)
{
...
network *parse_network_cfg(char *filename, int close_quantization)
{
    ...
    params.w = net->w;
    params.c = net->c;
    params.close_quantization = close_quantization;
    ...
    while(n){
        ...
        if(lt == CONVOLUTIONAL){
            ...
            l = parse_deconvolutional(options, params, count);
        }else if(lt == LOCAL){
            ...
            l = parse_yolo(options, params, count);
            ...
            l = parse_maxpool(options, params, count);
        }else if(lt == REORG){
            l = parse_reorg(options, params);
        }else if(lt == AVGPOOL){
            l = parse_avgpool(options, params);
        }else if(lt == ROUTE){
            l = parse_route(options, params, net, count);
        }else if(lt == UPSAMPLE){
            l = parse_upsample(options, params, net, count);
        }else if(lt == SHORTCUT){
            l = parse_shortcut(options, params, net, count);
    ...
    net->input_uint8 = calloc(net->inputs*net->batch, sizeof(uint8_t));
...
void load_convolutional_weights(layer l, FILE *fp, network *net, int index)
{
    if(l.binary){
    ...
    // if(l.activ_data_uint8_scales[0]){
    //     // l.min_input_value[0] = (QUANT_NEGATIVE_LIMIT - l.input_data_uint8_zero_point[0]) * l.input_data_uint8_scales[0];
    //     // l.max_input_value[0] = (QUANT_POSITIVE_LIMIT - l.input_data_uint8_zero_point[0]) * l.input_data_uint8_scales[0];
...
void load_maxpool_weights(layer l, FILE *fp, network *net, int index)
{
    fread(l.activ_data_uint8_scales, sizeof(float), 1, fp);
    fread(l.activ_data_uint8_zero_point, sizeof(uint8_t), 1, fp);
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    for(i = start; i < net->n && i < cutoff; ++i){
        ...
        if(l.type == CONVOLUTIONAL || l.type == DECONVOLUTIONAL){
            load_convolutional_weights(l, fp, net, i);
        }
        if(l.type == CONNECTED){
